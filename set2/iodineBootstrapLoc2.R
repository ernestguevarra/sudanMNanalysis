...
for(i in stateNames) {
  for(j in unique(locNames$locality[locNames$state == i])) {
    for(k in "Non-pregnant non-lactating") {
      for(l in params) {
...
for(i in sort(unique(subDF$localityID))) {
  ...
  for(j in "Non-pregnant non-lactating") {
...
indicatorName <- ifelse(yy[ , 4] == "ID2", "Mild iodine deficiency",
                   ifelse(yy[ , 4] == "ID3", "Moderate iodine deficiency", 
